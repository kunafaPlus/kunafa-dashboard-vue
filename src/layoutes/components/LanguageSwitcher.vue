<template>
  <div class="relative" :id="id">
    <button @click="toggleDropdown" class="flex items-center space-x-1 text-sm text-slate-800 dark:text-slate-100 hover:text-white bg-primary rounded px-3 py-2">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
      <div v-show="dropdownOpen" class="absolute z-10 mt-2 w-32 origin-top-right rounded-md shadow-lg right-0" ref="dropdown">
        <ul class="py-1 bg-primary">
          <li v-for="language in languages" :key="language.code">
            <button @click="changeLanguage(language)" class="block w-full px-2 py-2 text-sm text-center dark:text-slate-100 hover:text-white rounded-md">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const currentLanguage = computed(() => locale.value); // ar , en
