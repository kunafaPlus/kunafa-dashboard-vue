<template>
  <div>
    ...
    <div id="sidebar" ref="sidebar"
      class="flex flex-col absolute z-40 ltr:left-0 rtl:right-0 top-0 lg:static ltr:lg:left-auto rtl:lg:right-auto lg:top-auto lg:translate-x-0 h-screen overflow-y-scroll
    lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 shrink-0 bg-white dark:bg-slate-800 p-4 transition-all duration-200 ease-in-out"
      ...
      <div class="flex justify-between items-center mb-10 ltr:pr-3 rtl:pl-3 sm:px-2">
        ...
        <router-link class="flex items-center" to="/">
          ...
          <!-- System name -->
          <span class="text-xl font-semibold text-primary mx-2" v-if="sidebarExpanded">Kunafa Vue</span>
        </router-link>
      </div>
        ...
        <ul class="mt-3">
          <SidebarLinkGroup v-slot="parentLink" v-for="route in routes" :key="route.meta.label" class="rounded-2xl">
            ...
            <template v-if="route.children">
              <a class="block text-slate-600 truncate px-2 transition duration-150"
                :class="(currentRoute && currentRoute.fullPath.includes(route.meta.label)) ? 'hover:text-slate-200' : 'hover:text-slate-200'"
                  ...
                  <template v-for="child in route.children" :key="child.name">
                    <router-link :to="child.path" custom v-slot="{ href, navigate, isExactActive }">
                      <li class="mb-1 last:mb-0">
                        <a class="flex items-center block transition duration-150 truncate"
